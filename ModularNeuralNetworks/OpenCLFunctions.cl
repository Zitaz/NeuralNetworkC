...
__kernel void FeedForwardFCRelu(__global float* weights, __global float* values_in, __global float* values_out, int num_neurons_with_bias)
{
	...
	for(int y = 0; y < num_neurons_with_bias; y++)
	{
...
__kernel void FeedForwardFCSigmoid(__global float* weights, __global float* values_in, __global float* values_out, int num_neurons_with_bias)
{
	const size_t x_id = get_global_id(0);
	const size_t num_neurons_next = get_global_size(0);
...
__kernel void FeedForwardFCLinear(__global float* weights, __global float* values_in, __global float* values_out, int num_neurons_with_bias)
{
	const size_t x_id = get_global_id(0);
	const size_t num_neurons_next = get_global_size(0);
...
//	//for (size_t from_index = 0; from_index < num_neurons; from_index++)
//	//{
//	//	const size_t array_index = from_index * num_neurons_next + next_index;
//    //
...
__kernel void BackpropFCRelu(__global float* weights, __global float* delta_weights, __global float* gradient, __global float* next_gradient, __global float* values,
						 float eta, float momentum, int num_neurons_next)
{
	...
	for (size_t next_index = 0; next_index < num_neurons_next; next_index++)
	{
		const size_t array_index = this_index * num_neurons_next + next_index;
